...
export const createBooking = async (req: AuthRequest, res: Response): Promise<any> => {
  ...
  try {
    ...
    const scheduleResult = await pool.query(
      `SELECT r.price_ugx, r.name AS route_name, s.departure_time,
    ...
    const result = await pool.query(
      `INSERT INTO bookings
       ...
       RETURNING *`,
      [user_id, schedule_id, seat_number, total_amount, payment_method, qr_code]
    );
    ...
    await pool.query(
      'UPDATE bookings SET booking_reference = $1 WHERE id = $2',
    ...
    // Send booking confirmation push notification
     try {
      ...
      if (pushModule && typeof pushModule.notifyBookingConfirmed === 'function') {
        await pushModule.notifyBookingConfirmed(
          user_id,
          bookingRef,
          schedule.route_name,
...
export const cancelBooking = async (req: AuthRequest, res: Response): Promise<any> => {
  ...
  try {
    ...
    const check = await pool.query(
      `SELECT b.id, s.departure_time FROM bookings b
       JOIN schedules s ON b.schedule_id = s.id
...
// ─── GET BOOKED SEATS ─────────────────────────────────────────────
export const getBookedSeats = async (req: Request, res: Response): Promise<any> => {
...
export const verifyQrCode = async (req: AuthRequest, res: Response): Promise<any> => {
  const { qr_code } = req.body;
  try {
    const result = await pool.query(
      `SELECT b.*, u.full_name, u.phone, r.name AS route_name,
       s.departure_time
       FROM bookings b
...
export const getAllBookings = async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    const result = await pool.query(
