...
exports.saveFCMToken = exports.updateProfile = exports.getProfile = exports.verifyLoginOTP = exports.sendLoginOTP = exports.verifyOTPAndRegister = exports.sendOTP = void 0;
...
const sendOTP = async (req, res) => {
    ...
    try {
        ...
        const existing = await database_1.default.query('SELECT id FROM users WHERE phone = $1', [phone]);
...
const verifyOTPAndRegister = async (req, res) => {
    ...
    try {
        ...
        const result = await database_1.default.query(`INSERT INTO users (full_name, phone, role)
       VALUES ($1, $2, 'passenger')
       RETURNING id, full_name, phone, role`, [full_name, userPhone]);
...
const sendLoginOTP = async (req, res) => {
    ...
    try {
        ...
        const result = await database_1.default.query('SELECT id, full_name, phone FROM users WHERE phone = $1', [phone]);
...
const verifyLoginOTP = async (req, res) => {
    ...
    try {
        ...
        const result = await database_1.default.query('SELECT id, full_name, phone, role FROM users WHERE phone = $1', [phone]);
        if (result.rows.length === 0) {
...
const getProfile = async (req, res) => {
    try {
        const result = await database_1.default.query('SELECT id, full_name, phone, email, role, created_at FROM users WHERE id = $1', [req.user?.id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ message: 'User not found' });
        }
...
const updateProfile = async (req, res) => {
    ...
    try {
        const result = await database_1.default.query(`UPDATE users SET full_name = $1, email = $2
       WHERE id = $3
       RETURNING id, full_name, phone, email, role`, [full_name, email || null, req.user?.id]);
...
const saveFCMToken = async (req, res) => {
    ...
    try {
        await database_1.default.query('UPDATE users SET fcm_token = $1 WHERE id = $2', [token, req.user?.id]);
